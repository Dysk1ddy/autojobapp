...
export function scanPage(
  ...
  return buildScanSummary(rawFields, groups, fieldMatches, adapter, pageContext);
}
...
export async function fillPage(
  ...
  const scan = buildScanSummary(rawFields, groups, fieldMatches, adapter, pageContext);
  const autoSubmitResult = settings.autoSubmit
    ? attemptAutoSubmit(scan, results)
    : {
        autoSubmitted: false,
...
function fillMatchedGroup(
  ...
  if (!group) {
    return buildResult(match, "error", "", "Field group was not found during fill.");
  }

  if (!shouldFillConfidence(match.confidence, fillMode)) {
    return buildResult(
      match,
      "skipped",
      match.matchedValuePreview,
  ...
  if (!match.matchedKey) {
    return buildResult(
      ...
      "skipped",
      "",
      "No strong profile mapping was available."
    );
  }

  const resolvedValue = resolveProfileFieldValue(
    profile,
    match.matchedKey,
    repeatEntryIndex
  );

  if (!resolvedValue.hasValue || resolvedValue.raw === null) {
    return buildResult(
      match,
      "skipped",
  ...
  if (isAlreadyFilled(group, match, resolvedValue)) {
    return buildResult(
      match,
      "skipped",
      resolvedValue.preview,
  ...
  try {
    if (isChoiceGroup(group)) {
      const changed = fillChoiceGroup(group, resolvedValue);
    ...
    if (primaryElement instanceof HTMLSelectElement) {
      const changed = fillSelectControl(primaryElement, resolvedValue);
    ...
    const changed = fillTextControl(primaryElement, resolvedValue);
